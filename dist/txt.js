...
(function (txt) {
    var Character = (function (_super) {
        ...
        function Character(character, style, index, glyph) {
            ...
            else if (this._font.glyphs[String.fromCharCode(this.characterCode)
                .toLowerCase()
                .charCodeAt(0)]) {
                this._glyph = this._font.glyphs[String.fromCharCode(this.characterCode)
                    .toLowerCase()
                    .charCodeAt(0)];
            }
            else if (this._font.glyphs[String.fromCharCode(this.characterCode)
            ...
            ha.graphics
                .beginFill("#000")
                .drawRect(0, this._font.descent, this._glyph.offset * this._font.units, this._font.ascent - this._font.descent);
            this.hitArea = ha;
        }
    ...
    var FontLoader = (function () {
        ...
        FontLoader.loadFont = function (fontName, loader) {
            if (txt.FontLoader.fonts.hasOwnProperty(fontName)) {
                ...
                req.onload = function () {
                    if (localStorage &&
                        txt.FontLoader.cache &&
                        this.cacheFont == undefined) {
        ...
        Path.prototype.getPathPoint = function (distance, characterLength, charOffset) {
            ...
            if (segment == 4) {
                if (direction) {
                    ...
                    if (this.pathElement.getPathSegAtLength(position) !=
                        ...
                        if (d0 > d1) {
                            ...
                            var rot12 = (Math.atan((point2.y - point1.y) / (point2.x - point1.x)) *
                                180) /
                                Math.PI;
        ...
        PathText.prototype.characterLayout = function () {
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (hPosition == 0) {
                    hPosition =
                        this.initialOffset +
                            this.trackingOffset(this.initialTracking, currentStyle.size, txt.FontLoader.getFont(currentStyle.font).units);
                }
                char = new txt.Character(this.text.charAt(i), currentStyle, i);
                if (this.original.character) {
                    if (this.original.character.added) {
